...
pygame.display.set_caption("Orion's Rage")
clock = pygame.time.Clock()

font_name = pygame.font.match_font('arial')
def draw_text(surf, text, size, x, y):
    font=pygame.font.Font(font_name, size)
    #True below to get the text anti aliased
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #pygame.K_a for using a key and so on
...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        ...
        #above func gives time since start of game
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self,center,size):
        ...
        self.rect = self.image.get_rect()
        self.rect.center= center
        #below frame means image of explosion or frame of explosion
        self.frame = 0
        self.last_update=pygame.time.get_ticks()
        self.frame_rate=75
...
def show_go_screen():
    ...
    draw_text(screen, "Orion's Rage",64, WIDTH/2, HEIGHT/4)
    ...
    while waiting:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                #Keyup below means as soon as we release the key coz if it was key down game would start as soon as we press it
...
pygame.mixer.music.load(path.join(snd_dir,'bckgrnd.ogg'))
pygame.mixer.music.set_volume(0.4)


#-1 below means it loops from start again
...
while running:
    ...
    #if u don't tell it to check for circle, it will take rectangle by default
    hits=pygame.sprite.spritecollide(player,mobs,True,pygame.sprite.collide_circle)
    #False above means don't destroy
    for hit in hits:
